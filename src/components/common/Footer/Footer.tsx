          ...
          <ul className={css.socialsList}>
            {footer.data.socials.map((i, index) => (
              <li className={css.socialsItem} key={index}>
                <PrismicNextLink className={css.socialsLink} field={i.link}>
                  {i.icon === "instagram" && (
...
function FacebookIcon({ className }: { className?: string }) {
  return (
    ...
    >
      <path
        // fill="#fff"
        d="M10.071 15.456h4.653l.73-4.755H10.07V8.1c0-1.975.642-3.726 2.479-3.726H15.5V.225C14.982.155 13.885 0 11.812 0 7.486 0 4.949 2.3 4.949 7.537v3.164H.5v4.755h4.448v13.07c.881.134 1.774.224 2.69.224.828 0 1.636-.076 2.433-.185V15.456z"
      ></path>
    </svg>
  );
}
...
function TelegramIcon({ className }: { className?: string }) {
  return (
    ...
    >
      <path
        // fill="#fff"
        d="M24 .753l-3.757 19.612s-.525 1.36-1.97.708l-8.667-6.882-.04-.02c1.17-1.09 10.25-9.543 10.646-9.926.615-.593.233-.947-.48-.498l-13.411 8.82-5.174-1.803s-.814-.3-.893-.953c-.079-.653.92-1.006.92-1.006L22.266.236S24-.553 24 .753z"
      ></path>
    </svg>
  );
}
